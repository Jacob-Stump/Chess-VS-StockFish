...
public class Board implements MouseListener {
	...
	public void queenMove() { //mixture of rook and bishop logic
		int row = selectedPieceLocation[0];
		int col = selectedPieceLocation[1];
		int rowDest = destinationPieceLocation[0];
		int colDest = destinationPieceLocation[1];
		...
		if(destinationPiece.type != PieceType.KING) {	
			if(rowDest < row && colDest < col ) { //if the move is upper left
					while(queenRowUp >= 0 && queenColLeft >= 0) { 
						if(queenRowUp == rowDest && queenColLeft == colDest) {
							if(chessBoard[queenRowUp][queenColLeft].color == PieceColor.BLACK) {
								chessBoard[row][col] = nul;
								chessBoard[queenRowUp][queenColLeft] = selectedPiece;
								int index = pieceBox.indexOf(destinationPiece);
								pieceBox.remove(index);
								
							}
							else if(chessBoard[queenRowUp][queenColLeft].color == PieceColor.NONE) {
								chessBoard[row][col] = nul;
								chessBoard[queenRowUp][queenColLeft] = selectedPiece;
							}
							break;
						}
						else if(chessBoard[queenRowUp][queenColLeft].color == PieceColor.WHITE || chessBoard[queenRowUp][queenColLeft].color == PieceColor.BLACK) {
							break;
						}
						queenRowUp--;
						queenColLeft--;
			...
			if(rowDest < row && colDest > col) { //if the move is upper right
					while(queenRowUp >= 0 && queenColRight < 8) { 
						if(queenRowUp == rowDest && queenColRight == colDest) {
							if(chessBoard[queenRowUp][queenColRight].color == PieceColor.BLACK) {
								chessBoard[row][col] = nul;
								chessBoard[queenRowUp][queenColRight] = selectedPiece;
								int index = pieceBox.indexOf(destinationPiece);
								pieceBox.remove(index);
								
							}
							else if(chessBoard[queenRowUp][queenColRight].color == PieceColor.NONE) {
								chessBoard[row][col] = nul;
								chessBoard[queenRowUp][queenColRight] = selectedPiece;
							}
							break;
						}
						else if(chessBoard[queenRowUp][queenColRight].color == PieceColor.WHITE || chessBoard[queenRowUp][queenColRight].color == PieceColor.BLACK) {
							break;
						}
						queenRowUp--;
						queenColRight++;
			...
			if(rowDest > row && colDest > col) { //if the move is lower right
					while(queenRowDown < 8 && queenColRight < 8) { 
						if(queenRowDown == rowDest && queenColRight == colDest) {
							if(chessBoard[queenRowDown][queenColRight].color == PieceColor.BLACK) {
								chessBoard[row][col] = nul;
								chessBoard[queenRowDown][queenColRight] = selectedPiece;
								int index = pieceBox.indexOf(destinationPiece);
								pieceBox.remove(index);
								
							}
							else if(chessBoard[queenRowDown][queenColRight].color == PieceColor.NONE) {
								chessBoard[row][col] = nul;
								chessBoard[queenRowDown][queenColRight] = selectedPiece;
							}
							break;
						}
						else if(chessBoard[queenRowDown][queenColRight].color == PieceColor.WHITE || chessBoard[queenRowUp][queenColRight].color == PieceColor.BLACK) {
							break;
						}
						queenRowDown++;
						queenColLeft++;
			...
			if(rowDest > row && colDest < col) { //if the move is lower left
				if(destinationPiece.type != PieceType.KING) {
					while(queenRowDown < 8 && queenColLeft >= 0) { 
						if(queenRowDown == rowDest && queenColLeft == colDest) {
							if(chessBoard[queenRowDown][queenColLeft].color == PieceColor.BLACK) {
								chessBoard[row][col] = nul;
								chessBoard[queenRowDown][queenColLeft] = selectedPiece;
								int index = pieceBox.indexOf(destinationPiece);
								pieceBox.remove(index);
								
							}
							else if(chessBoard[queenRowDown][queenColLeft].color == PieceColor.NONE) {
								chessBoard[row][col] = nul;
								chessBoard[queenRowDown][queenColLeft] = selectedPiece;
							}
							break;
						}
						else if(chessBoard[queenRowDown][queenColLeft].color == PieceColor.WHITE || chessBoard[queenRowDown][queenColLeft].color == PieceColor.BLACK) {
							break;
						}
						queenRowDown++;
						queenColLeft--;
